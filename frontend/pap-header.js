/**
 * PAP Magazine — Unified Header Component (Pattern A)
 *
 * Drop-in script for pages that do NOT already have the main-page header
 * (sub-pages, community, mypage, etc.).
 *
 * What it does:
 *  1. Injects header CSS (only if pap-styles.css is not loaded)
 *  2. Removes legacy navigation (side-nav, c-header, mp-back, etc.)
 *  3. Injects the Pattern-A header + nav-overlay + search-overlay
 *  4. Provides toggleNav / toggleSearch / toggleAccountMenu if not defined
 *  5. Adjusts page layout (content padding) for the top header
 */
(function () {
  'use strict';

  /* ── guard: skip if the page already has Pattern-A header ── */
  ...
  /* ================================================================
     2. Layout fix — inject CSS to adjust page content
     ================================================================ */
  ...
  document.body.classList.add('pap-has-header');

  /* ================================================================
     3. HTML — build and inject
     ================================================================ */
  function _navLink(url, i18nKey, label, extraStyle) {
    var s = extraStyle ? ' style="' + extraStyle + '"' : '';
    return '<a href="#" onclick="event.preventDefault();_papCloseNav();' +
      (typeof navigateWithInterstitial === 'function' ? 'navigateWithInterstitial(\'' + url + '\')' : 'window.location.href=\'' + url + '\'') +
      ';" data-i18n="' + i18nKey + '"' + s + '>' + label + '</a>';
  }
  ...
  /* Alias for pages that call toggleNav() / closeNav() */
  if (typeof window.toggleNav !== 'function') window.toggleNav = window._papToggleNav;
  if (typeof window.closeNav !== 'function') window.closeNav = window._papCloseNav;
  ...
  if (typeof window.toggleSearch !== 'function') window.toggleSearch = window._papToggleSearch;
  ...
  if (typeof window.toggleAccountMenu !== 'function') window.toggleAccountMenu = window._papToggleAccount;
  ...
  /* ================================================================
     5. Sync language selector with page state
     ================================================================ */
