...
function _baseFeeApprovalBlock(submissionId, submissionType, paymentStatus){
  ...
  // 2026-09-13 — 연간 프리미엄 €380 면제(서버 payment_status='waived'): 결제 버튼 대신 면제 안내.
  if(paymentStatus==='waived'){
    return '<div style="margin:0 0 16px;padding:14px 16px;background:rgba(201,168,106,.08);border:1px solid rgba(201,168,106,.35);border-radius:2px">'+
             '<div style="font-size:12px;font-weight:700;color:#c9a86a;letter-spacing:.04em;margin-bottom:4px">✓ '+_payT('payBaseWaived')+'</div>'+
             '<div style="font-size:12px;color:rgba(255,255,255,.6)">'+_payT('payBaseWaivedHint')+'</div>'+
           '</div>';
  }
