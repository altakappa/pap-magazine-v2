...
// 베타 기간 중에는 로그인한 모든 회원에게 전체 접근 권한 부여
function isPremium(){
  try{
    if(isBetaActive() && isLoggedIn()) return true;
