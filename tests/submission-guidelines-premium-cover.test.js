...
// 2026-09-12 도메니코 2차: 혜택 섹션에 커버 선택은 쓰지 않고, 공동작업자 혜택을 명시. (2026-09-24: "연간 프리미엄이 5명 지정, 무료 회원 포함" 으로 바뀜)
...
ok('정적 마크업(영어 기본값)도 6장·공동작업자 혜택·종이 잡지', /<li>At least 6 high-resolution photos/.test(html) && /glAddBody"><ul>(?:<li>[^<]*<\/li>)*<li>Yearly Premium: choose up to 5 PAP members \(free members included\) as Instagram collaborators on your editorial's post\. Each person must be a PAP member[^<]*until the review is finished<\/li>/.test(html) && /glTermsBody"><ul><li>[^<]*<\/li><li>Submitted work may be published in a future PAP print magazine/.test(html));
...
ok('제7조 ⑦: 옷 브랜드 3개·룩 3개, 잡화 제외, 미충족 시 €380·단일 브랜드 €790 — 9개 언어', (terms.match(/⑦ (?:[^'\\]|\\.)*380(?:[^'\\]|\\.)*(?:<\/li>|<br>)'/g) || []).length === 9 && (terms.match(/⑦ (?:[^'\\]|\\.)*(액세서리|accessories|Accessoires|accessori|accessoires|accesorios|アクセサリー|配饰|аксессуаров)/g) || []).length === 9);
...
console.log('\n=== 제7조 ②·⑧ 브랜디드 판정은 객관적 기준 — 의도·상업성·홍보 목적 무관, 예외 없음 (도메니코 2026-09-14) ===');
ok('제7조 ②: "홍보를 주된 목적" 문구 제거 → 크레딧상 브랜드 구성만으로 판정 (9개 언어)', !/홍보를 주된 목적으로|primary purpose of promoting|primär werblichem|scopo principale la promozione|visant principalement la promotion|objetivo principal sea la promoción|プロモーションを主目的|推广特定品牌为主要目的|имеющие целью продвижение/.test(terms) && (terms.match(/② (?:[^'\\]|\\.)*(오직 제출된 크레딧상의 브랜드 구성|solely by the brand composition|ausschließlich nach der Markenzusammensetzung|esclusivamente sulla composizione dei brand|uniquement par la composition des marques|únicamente por la composición de marcas|ブランド構成のみで判断|仅根据所提交署名中的品牌构成|исключительно составом брендов)/g) || []).length === 9);
ok('제7조 ⑧: 의도·상업성·홍보 목적·협찬 무관 + 재검토·예외 요청 불가 + 재제출 안내 — 9개 언어', (terms.match(/⑧ (?:[^'\\]|\\.)*(예외 적용을 요청할 수 없습니다|may not request a review of the determination or an exception|eine Ausnahme verlangen|né un\\'eccezione|ni exception pour|ni una excepción|例外の適用を求めることはできません|申请例外|исключения по перечисленным основаниям)(?:[^'\\]|\\.)*<\/li>'/g) || []).length === 9 && (terms.match(/⑧ (?:[^'\\]|\\.)*(상업적 목적|commercial purpose|kommerziellen Zwecks|scopo commerciale|but commercial|fin comercial|商業目的|商业目的|коммерческой цели)/g) || []).length === 9);
ok('가이드라인 이용 약관 섹션에도 같은 원칙 한 줄 (9개 언어, 영어는 기본 사전에도 한 번 더)', ['브랜디드 판정은 크레딧상의 브랜드 구성만으로','The branded determination is based solely','Die Branded-Einstufung richtet sich allein','La valutazione branded si basa solo','La qualification branded repose uniquement','La calificación branded se basa únicamente','ブランデッド判定はクレジットのブランド構成のみで','品牌内容判定仅依据署名中的品牌构成','Отнесение к брендированному контенту определяется только'].every((t) => html.indexOf('<li>' + t) !== -1));
ok('정적 마크업 Article 7 ② 객관적 기준 + ⑧, 요약 em 갱신, 약관 v6, 시행일 2026-09-14', /② Whether an editorial is Branded Content is determined solely by the brand composition/.test(html) && /⑧ The Branded Content determination is an objective standard/.test(html) && !/prior consultation with PAP/.test(html) && /submission-terms\.js\?v=[6-9]/.test(html) && count(/termsEffective:'[^']*(9월 14일|September 14, 2026|14\. September 2026|14 settembre 2026|14 septembre 2026|14 de septiembre de 2026|9月14日|14 сентября 2026)/g) === 9);
ok('가이드라인: 무료 조건 줄에 "불충족 시 유료 게재비 (2개 이하 €380, 1개 €790)" 9개 언어 (도메니코 2026-09-14)', ['불충족 시 유료 게재비가 부과됩니다 (옷 브랜드 2개 이하 €380, 1개 €790)','If not met, a paid publication fee applies (2 or fewer clothing brands €380, 1 brand €790)','Andernfalls fällt eine kostenpflichtige Veröffentlichungsgebühr an','In caso contrario si applica una tariffa di pubblicazione a pagamento','À défaut, des frais de publication payants','Si no se cumple, se aplica una tarifa de publicación de pago','満たさない場合は有料掲載料が発生します','不满足时将收取付费刊登费','Иначе взимается плата за платную публикацию'].every((t) => html.indexOf(t) !== -1));
