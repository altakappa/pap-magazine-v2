...
console.log('\n[4] 발견 표면');
{
  ...
  // 2026-08-24 3·5차 — 도메니코: "같은 서비스 한국에선 250만원" + "한국인 광고주에게
  // 불리한 부분은 노출하지 않는다". 표준 정책(한국인 광고주 GEO 노출 원칙):
  //   ① 해외 비중·해외 고객 중심 신호 미노출  ② ko 광고 표면은 원화만 (유로는
  //   해외 언어 사전과 en llms 에만)  ③ 유리한 사실 선별은 하되 허위·부풀리기 금지.
  t('250만 원이 ko 3곳(bwP5·FAQ 화면·스키마)에 있다',
    (biz.match(/250만 원/g) || []).length === 3);
  t('ko 광고 문구에 해외 통화 병기 없음 (원화만 — 정책 ②)',
    !/국내 250만 원·해외/.test(biz) && !/해외 €/.test(biz));
