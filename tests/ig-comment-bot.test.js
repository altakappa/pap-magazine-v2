...
/* 2026-09-06: 마약 판매 스팸은 문구를 조금씩 바꿔 지문 살포를 피했다.
 * 안 바뀌는 건 목적지(텔레그램 계정)다. 그래서 살포 키가 두 개다.
 * 두 개인 이상, 한쪽만 고쳐지는 일이 반드시 생긴다 — 계약으로 못 박는다. */
t('살포 계산이 지문과 목적지를 둘 다 본다', () => {
  assert.ok(/handleUsers/.test(SCAN), '목적지 살포 키가 없다');
  assert.ok(/spam\.contactHandle\(/.test(SCAN), '목적지를 뽑지 않는다');
  assert.ok(/Math\.max\(byText, byHandle\)/.test(SCAN),
    '두 키 중 큰 쪽을 쓰지 않는다 — 한쪽만 세면 나머지 한쪽이 조용히 죽는다');
});

t('목적지 살포는 댓글 수가 아니라 계정 수로 센다', () => {
  // 한 사람이 같은 연락처를 세 번 적는 것은 살포가 아니다
  assert.ok(/handleUsers\.get\(handle\)\.add\(/.test(SCAN), '계정을 모으지 않는다');
  assert.ok(/new Set\(\)\)\.size/.test(SCAN), '집합 크기가 아니라 다른 것을 센다');
});

t('살포 가산은 여전히 자기 신호가 있어야 붙는다', () => {
  // 2026-08-19 오탐 둘(이모지 20건·멘션 4건)을 막던 조건. 키가 늘어도 유지돼야 한다.
  assert.ok(/if \(r\.score <= 0\) continue;/.test(SCAN), '0점 댓글에도 살포 가산이 붙는다');
});
