 ...
 *
 * 2026-08-13 지시 (변경):
 *   3. **독자 호칭을 없앤다.** 그전에는 '너' 를 '패퍼들' 로 바꿨는데,
 *      실제 반응 좋은 스레드 글은 독자를 아예 부르지 않는다. 주어를
 *      지우고 바로 말한다. 호칭이 붙으면 방송 말투가 되고 광고로 읽힌다.
...
console.log('\n=== 1. 주어 자리의 호칭은 지워진다 (부르지 않는다) ===');
t('너는 → (삭제)', N('너는 어떤 쪽이야?') === '어떤 쪽이야?', N('너는 어떤 쪽이야?'));
t('너도 → (삭제)', N('너도 봤어?') === '봤어?', N('너도 봤어?'));
t('너가 → (삭제)', N('너가 골라봐') === '골라봐', N('너가 골라봐'));
t('문장 중간에서도 지워지고 두 칸이 남지 않는다',
  N('그래서, 너는 어때') === '그래서, 어때', JSON.stringify(N('그래서, 너는 어때')));
t('너희는 / 너네는 도 지워진다',
  N('너희는 어때') === '어때' && N('너네는 어때') === '어때',
  N('너희는 어때') + ' / ' + N('너네는 어때'));

console.log('\n=== 2. 지우면 문장이 무너지는 자리는 "다들"로 ===');
...
t('패퍼들은 → (삭제)', N('패퍼들은 어떻게 생각해?') === '어떻게 생각해?',
  N('패퍼들은 어떻게 생각해?'));
t('패퍼들 → 다들', N('패퍼들 생각은?') === '다들 생각은?', N('패퍼들 생각은?'));
t('페퍼들(페퍼릿 표기)도 같이 잡는다', N('페퍼들 생각은?') === '다들 생각은?');
t('프롬프트에 패퍼들 지시가 남아 있지 않다',
  !/독자를 부를 때는 "패퍼들"/.test(voice),
...
t('프롬프트가 "독자를 부르지 않는다"를 지시한다',
  /독자를 부르지 않는다/.test(voicePrompt),
