...
console.log('=== 어드민 버튼도 실제 동작을 말한다 ===');
{
  ...
  // 2026-08-13 실측: 버튼 글자는 고쳤는데 마우스를 올리면 뜨는 설명(title)은
  //   "게재료 결제요청이 표시됩니다" 그대로였다. 눈에 잘 안 띄는 문구가 오래
  //   살아남는다. 주석도 같이 센다 — 틀린 주석은 다음 사람에게 틀린 코드를 부른다.
  ok("'결제요청' 이라는 말이 파일 전체에 0건이다",
    admin.indexOf('결제요청') === -1,
    '툴팁(title)·주석까지 포함해서 남아 있으면 안 된다');
  ok('툴팁이 청구를 말한다',
    /_apBtn\.title='승인하면 묶어둔 게재료가 즉시 청구됩니다'/.test(admin));
}
