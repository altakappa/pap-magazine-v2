...
/* 2026-09-02 — 이 단언이 문구('애매')를 고정하고 있었다. 진짜 의도는
   **거부 사유에 후보 기사 제목이 담겨서 사람이 고를 수 있어야 한다** 이다.
   문구는 그대로 두면 노트를 읽는 사람이 "버그인가?" 하고 헷갈려서
   '사람이 골라야 한다' 로 바꿨다. 의도를 검사하도록 고친다. */
t('거부 사유에 후보 기사 제목이 담긴다 (사람이 고를 수 있어야 한다)',
  /베이델리/.test(amb.reason), amb.reason);
t('거부 사유가 "사람이 판단할 일" 임을 말한다', /사람이 골라야 한다/.test(amb.reason), amb.reason);
