...
console.log('\n=== ⑩ 자체 취재 판별 (🎥 PAP) ===');
{
  ...
  // 2026-08-27 — AI 브리핑 인용 최적화 블록 (도메니코 제안: 블로그도 GEO 구조로)
  t('AI 인용 최적화 블록이 프레임워크에 있다', /AI 인용 최적화/.test(draftSrc));
  t('답 먼저 규칙 — 도입 200자 내 핵심 답', /첫 200자에서 답을 찾는다/.test(draftSrc));
  t('질문형 소제목 + 구간 첫 문장이 답', /질문형/.test(draftSrc) && /첫 1~2문장이 곧 답/.test(draftSrc));
  t('Q&A 2문항 규칙 (본문 재정리만, 새 주장 금지)', /자주 묻는 질문/.test(draftSrc) && /새 주장 금지/.test(draftSrc));
  t('표현 획일화 금지 (유사문서 방지)', /유사문서로 묶여/.test(draftSrc));
