...
/* 리사이즈 (2026-08-14 신설) — 그동안 30MB 상한을 넘는 구 S3 원본 17장(58~98MB)이
   '용량 초과'로 영구 제외돼 있었다. 원본은 멀쩡한데 우리 상한 때문에 못 받은 것이다.
   코드 주석이 "리사이즈가 답이고 별건"이라고 미뤄둔 그 별건을 여기서 지킨다. */
t('상한 초과분을 sharp 로 줄여서 받는다',
  /require\('sharp'\)/.test(mig) && /HARD_MAX_BYTES/.test(mig),
  '원본이 살아 있는데 우리 상한 때문에 버리면 안 된다');
t('리사이즈에도 천장이 있다 (메모리 보호)',
  /HARD_MAX_BYTES = 120 \* 1024 \* 1024/.test(mig),
  '함수 메모리 1024MB — 원본 버퍼와 sharp 작업분을 함께 든다');
t('gif 는 리사이즈하지 않는다',
  /gif\/i\.test\(ct\)/.test(mig),
  '애니메이션이 깨진다 — 종전대로 실패 처리가 맞다');
t('줄인 뒤에도 상한을 넘으면 실패로 남긴다',
  /too large after resize/.test(mig),
  '조용히 통과시키면 스토리지에 거대 파일이 쌓인다');
t('큰 파일은 예산 남았을 때만 시도한다',
  /BIG_FILE_HOSTS\.test\(url\)[\s\S]{0,80}TIME_BUDGET_MS \* 0\.4/.test(mig),
  '98MB 한 장이 90초 예산을 다 먹으면 그 회차는 한 편도 못 끝낸다');

/* 알림 가독성 (2026-08-14) — 종전 알림은 '· HTTP 404 — editorial ffbd8595-…' 를
   8줄 반복했다. 도메니코가 "같은 게 8번 나오는데 이게 뭐냐"고 물었고, 실제로는
   한 화보(ROOM CARTEL)의 12장이었다. UUID 만으로는 어느 화보인지 알 수 없다. */
t('알림에 화보 제목을 붙인다',
  /select\('id, title'\)/.test(mig) && /titleOf\[/.test(mig),
  'UUID 앞 8자로는 어느 화보인지 모른다');
t('같은 화보의 실패를 묶어서 보여준다',
  /groupByEditorial/.test(mig),
  '12장이 12줄로 흩어지면 진짜 규모가 안 보인다');
t('누적 미해결 건수를 함께 알린다',
  /누적 미해결/.test(mig),
  '오늘 것만 알리면 두 달치가 매일 12건 뒤에 가려진다');
