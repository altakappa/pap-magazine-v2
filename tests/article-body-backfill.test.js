...
console.log('\n=== ⑤ 검수기 ===');
{
  ...
  /* 2026-09-14 — 목표 800 → 1,200자. 28일 실측에서 관계가 분명했다:
     812자 → 3.5위 CTR 21.3% / 405자 → 6.5위 CTR 0.18%.
     1차 25편이 평균 763자에 그쳐 3.5위 그룹에 못 미쳤다. 근거(사진)를 3장 → 6장으로
     늘려 지어내지 않고도 길어지게 했다. */
  t('1,200자 미만이면 잡는다',
    c('짧다.', '가'.repeat(500) + '다.').some((i) => /1200자 미만/.test(i)));
  t('제대로 늘어나면 통과 (1,200자 이상)',
    c(short, '가'.repeat(400) + '다.\n\n나'.repeat(1) + '나'.repeat(400) + '다.\n\n' + '다'.repeat(450) + '다.').length === 0);
  ...
  /* 2026-08-22 — 기사 규격이 3~4단락 → 4~5단락 으로 바뀌었다(600~800자).
     린트 한도도 같이 5단락으로 올린다. 어긋나면 정상 본문이 위반으로 잡힌다. */
  /* 2026-09-14 — 목표 자수를 1,200 으로 올리며 단락 한도도 5 → 7 로 올렸다.
     프롬프트 목표만 올리고 린터를 두면, 늘어난 본문이 전부 위반으로 잡혀
     사람이 반려하게 된다. 실제로 maxLen 1100 이 진짜 천장이었다. */
  t('기사 한도(7단락)를 넘기면 잡는다',
    c(short, long + '\n\n라다.\n\n마다.\n\n바다.\n\n사다.\n\n아다.').some((i) => /단락/.test(i)));
  t('7단락은 잡지 않는다 (새 규격의 정상 범위)',
    !c(short, long + '\n\n라다.\n\n마다.\n\n바다.\n\n사다.').some((i) => /단락/.test(i)));
...
console.log('\n=== ⑦ 안전 규칙 ===');
{
  ...
  /* 2026-09-14 — 3장 → 6장. 모델이 게을러서 짧았던 게 아니라 쓸 근거가 3장뿐이었다.
     재료를 늘리는 쪽이 "길게 써라" 보다 안전하다 (지어내게 만들지 않는다). */
  t('이미지는 6장까지만 (함수 시간 보호)', /VISION_MAX \|\| 6/.test(src) && /slice\(0, VISION_MAX\)/.test(src));
