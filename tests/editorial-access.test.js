...
t('스탠다드 열람 범위 표기가 두 화면에서 어긋나지 않는다', () => {
  ...
  /* 2026-09-21 도메니코: "6개월" 은 틀린 말이다 — 규칙은 현재 분기 + 직전 두 분기(6~9개월). 두 화면 다 같은 새 표기. */
  assert.ok(/최근 3개 분기 에디토리얼 열람\(약 6~9개월\)/.test(MY), '마이페이지 ko 표기가 "최근 3개 분기" 가 아니다');
  assert.ok(/최근 3개 분기 에디토리얼 열람\(약 6~9개월\)/.test(SUB), '구독 페이지 ko 표기가 "최근 3개 분기" 가 아니다');
  assert.ok(!/최신 6개월/.test(MY) && !/최신 6개월/.test(SUB), '"최신 6개월" 이 아직 남아 있다');
});
