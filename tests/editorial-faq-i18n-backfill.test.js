 ...
 * 도메니코 판정: 2026-08-27 "최근분만" → 2026-09-02 "전량". 설명문은 건드리지 않는다.
 */
...
/* 2026-08-27 판정은 "최근 300편만" 이었고 이 줄은 그 '300' 을 원문으로 박아
   두고 있었다. 2026-09-02 에 도메니코가 전량으로 바꾸자 이 단정이 그 변경을
   막았다 — 판정을 코드 모양으로 굳히면 판정이 바뀔 때 테스트가 방해가 된다.
   봐야 할 것은 '범위를 손으로 조절할 길이 남아 있는가' 다. (§G) */
t('범위를 env 로 조절할 수 있다 (되돌릴 길)',
  /EDITORIAL_FAQ_I18N_RECENT/.test(lib));
t('기본 범위가 전량이다 (2026-09-02 도메니코 판정)', (() => {
  const m = /EDITORIAL_FAQ_I18N_RECENT \|\| '(\d+)'/.exec(lib);
  return !!m && Number(m[1]) === 0;   // 0 = 무제한
})());
...
/* 종전에는 catch 블록의 **생김새**를 봤다. 2026-09-02 에 언어를 동시에 부르게
   되면서 그 catch 가 Promise.all 앞으로 옮겨갔고, 동작은 그대로인데 단정이 깨졌다.
   봐야 할 것은 '한 언어가 던져도 실패로 표시하고 계속 간다' 는 사실이다.
   (노트에 ':실패' 가 찍히는지는 faq-i18n-jsonl.test.js 가 실제 실행으로 확인한다.) */
t('한 언어 실패가 나머지를 막지 않는다',
