 ...
 *
 * pretrain 열은 학습 크롤러 정책의 장기 효과를 재는 **유일한 계기**다.
 * 2026-08-22 학습봇 10개 차단(비용) → 2026-09-10 부분 재개방(도메니코 결정,
 * Amazonbot·Bytespider 만 계속 차단). 주의: 이 열은 16문항 표본이라 1건 차이가
 * 6%p 다. 한 회차 등락은 잡음이고, 모델 학습 데이터는 수개월 단위로 갱신되니
 * 정책 변경의 효과는 분기 단위로만 읽는다. 2주 뒤 등락으로 판단하지 않는다.
 *
...
function renderSovMd(rep) {
  ...
  if (rep.pretrain && rep.pretrain.sov != null && rep.pretrain.prevSov != null
    ...
    L.push('');
    L.push('> ⚠️ **학습 레이어가 내려갔다.** 16문항 표본이라 1건 차이가 6%p 다.'
      + ' 두 회차 연속 내려가도 잡음일 수 있다. 학습봇은 2026-09-10 부터 다시 열려 있고'
      + '(robots.txt, Amazonbot·Bytespider 만 차단) 학습 데이터는 수개월 단위로 갱신되니'
      + ' 이 열은 분기 추세로만 읽을 것.');
  }
  L.push('');
  return L.join('\n');
