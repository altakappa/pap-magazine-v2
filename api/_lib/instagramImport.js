...
async function generateArticleFromPost(post, opts){
  ...
  const promptLines = [
    ...
    // 2026-08-26 — kind 를 반드시 붙인다. 셀럽 브리프가 이 값으로 거른다
    ...
    // 2026-08-26 — 도메니코: "제발 셀럽소식만 보내줘. 셀럽이 매거진에 실린소식은
    //   안알려줘도돼. 챌린지도 알려줄필요없어." 셀럽 브리프가 이 값으로 거른다.
    //   (2026-08-26 정정: brand_campaign 은 그 자체로 막지 않는다. 인물이 있으면 통과.)
    '  "brief_topic": "(아래 다섯 중 정확히 하나. 캡션·이미지에서 확인되는 것만.)",',
    '    // celeb_news       — 셀럽의 활동·발표·컴백·행사·공항·근황·수상·광고 발탁',
    '    // magazine_feature — 셀럽이 잡지·화보·커버에 실린 소식 (매거진 발행 소식)',
    '    // challenge        — 댄스·해시태그 챌린지 참여 안내나 릴레이',
    '    // brand_campaign   — 브랜드 캠페인·제품·룩북 (셀럽이 나오면 entities 에 반드시 적을 것)',
    '    // other            — 위 어디에도 안 맞음',
