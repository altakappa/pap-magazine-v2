...
/* 2 → 3 (2026-09-02 2차 상향). 앞서 3 에서 실패했던 건 **batch 18** 때문이었지
   동시 수 때문이 아니었다(로그: aborted due to timeout, 429 는 0건).
   batch 8 로 내린 뒤 실측: duration 88,349ms · 2회전 = 파도당 약 44초, 콜 상한 55초.
   콜들은 서로 기다리지 않으므로 하나를 더 붙여도 파도 길이는 거의 그대로다.

   되돌리는 법: 노트에 ':실패' 가 다시 보이면 2 로 내린다. 파도 하나를 잃을 뿐
   데이터는 안 잃는다. 감시(pipeline-watch)가 실패 비율 절반 넘으면 울린다. */
const CONCURRENCY = 3;
