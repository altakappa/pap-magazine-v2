...
function judgeLaneHealth(o) {
  ...
  /* ■ DB 세기와 생산자 세기의 범위가 다르다 (2026-09-05 실측)
     ...
     **발행됐고 원본 한국어 FAQ 가 있는 화보만** 채울 수 있다.

     ⚠️ 2026-09-05 에 나는 그 차이를 '원본 FAQ 가 없는 화보' 라고 적었다. 틀렸다.
        조인해서 세어 보니(09-06) 이렇다.
          draft 화보 220건의 번역행   886
          삭제된 화보 3건의 고아 행    11
          발행됐는데 원본 없음          0   ← 하나도 없다
        발행 화보 2,297건은 **전부** 원본 FAQ 를 갖고 있다. 백필이 못 채우는 게
        아니라 **아직 발행 안 한 화보의 자리**다. 발행하면 저절로 순서가 돈다.
        숫자가 다른 걸 보고 원인을 확인하지 않고 짐작으로 적은 결과다.
  ...
  if (noteRemaining === 0) {
    return { ...base, status: 'done', healthy: true,
      reason: label + ' 완주 — 생산자 기준 남은 칸 0'
        + (remaining > 0 ? ' (DB 에 ' + remaining + '칸이 남아 있지만 아직 발행 안 한 화보의 자리다 — 발행하면 저절로 채워진다)' : '') + '.' };
  }
  if (runs === 0) {
