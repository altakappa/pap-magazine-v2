...
const X_VOICE = [
  ...
  '  ※ 하지 말라는 표현을 예시로 적지 않는다. 모델이 그 문구를 오히려 집는다.',
  // 2026-08-13 도메니코 확정 — 스레드와 같은 호칭을 쓴다.
  '- 독자를 부를 때는 "다들"이라고 한다. 2인칭 대명사는 쓰지 않는다.',
  '  예: "다들 어떻게 생각하세요?"',
  '  매 글마다 부르지는 않는다. 부를 자리가 아니면 호칭 없이 그냥 말한다.',
  '- 질문으로 끝내는 것은 기본값이 아니다. 정말 답이 궁금한 지점이 있을 때만 묻는다.',
 ...
 *
 * 2026-08-13 도메니코 확정 — 독자 호칭은 **"다들"**.
 * 그전(2026-08-03)에는 "패퍼들"이었다. 커뮤니티 호칭은 안쪽 사람에게만
 * 통하고, 스레드처럼 우리를 모르는 사람에게도 닿는 자리에서는 방송 말투로
 * 읽힌다. "다들"은 부르되 가두지 않는다.
 *
 * 규칙은 하나다: 2인칭과 옛 호칭을 전부 "다들"로 바꾼다.
 *   - 주어·주제 자리(너는/너도/너가 …)는 조사 없이 그냥 "다들".
 *     "다들은/다들도"는 어색하다. 한국어에서 "다들"은 조사를 잘 안 붙인다.
 *   - 나머지 격조사 자리(너를/너의/너한테 …)만 "다들+조사".
 *
