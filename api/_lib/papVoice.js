...
/* ── 2026-08-22 — 두 번 틀리고 나서 정리한 규격 ──────────────────
 *
 * ■ 1차 진단 (08-20, 틀림): "재료가 부족하다" → 비전 이미지 1→4장.
 *   결과 500 → 525자. 25자. 이미지는 병목이 아니었다.
 *
 * ■ 2차 진단 (08-22 낮, 절반만 맞음): "규격의 산수가 안 맞는다".
 *   단락 3~4개 × 3~5문장 × 짧은 문장으로는 800자가 도달 불가능한 건 맞다.
 *   그래서 5~6단락으로 올렸다. 그런데 실제 캡션으로 써 보니 671자였다.
 *   문장이 40자일 거라 가정했는데 PAP 문체의 실측은 **33자**다
 *   ("문장은 짧게 끊는다" 가 같은 규격 안에 있다). 또 산수를 틀렸다.
 *
 * ■ 진짜 원인 (08-22 밤): **캡션이 이중언어다.**
 *
 *     캡션 전체 평균      1,044자
 *     그중 한국어 부분      350자   ← 나머지는 영문 번역·핸들·해시태그
 *     본문 평균            475자
 *     본문 ÷ 한국어캡션    **1.49**
 *
 *   모델은 압축한 적이 없다. 이미 재료를 1.5배로 늘려 쓰고 있었다.
 *   앞서 "캡션 1,500자를 476자로 압축(비율 0.30)" 이라고 진단한 것은
 *   **영문 중복분을 한국어 재료로 세어서** 나온 허수다.
 *
 *   한국어 재료 350자에서 800자를 뽑으라는 것은 2.3배로 부풀리라는 뜻이고,
 *   그 방법은 지어내기 아니면 같은 말 반복뿐이다 — 이 규격이 금지한 바로 그것.
 *   **목표가 재료를 못 이긴다.** 그래서 목표를 재료에 맞춘다.
 *
 * ■ 지금 규격: 600~800자 (도메니코 승인 2026-08-22)
 *     실측 문장 33자 기준 → 4~5단락 × 5문장 = 660~825자.
 *   800자 이상이 필요하면 고칠 곳은 프롬프트가 아니라 **재료**다
 *   (인스타 한국어 캡션을 길게 쓰거나, 웹 전용 정보 블록을 따로 넣는다).
 *   그 설계 전까지 여기 숫자를 올리지 마라 — 올리면 지어내기 압력만 는다. */
const LENGTH_ARTICLE = [
  '- 단락은 4~5개. 6단락 이상 금지.',
  '- 한 단락은 5문장 안팎(4~6문장). 문장은 짧게 끊는다.',
  '- 전체 600~800자(공백 포함). 1,000자 초과 금지.',
  ...
  '  쓸 내용이 정말 없으면 단락 수를 줄여라. 600자에 못 미쳐도 된다. 지어내는 것보다 짧은 게 낫다.',
  /* 압축 금지 규칙은 유지한다 (2026-08-22). 재료가 1.5배로 늘려 쓰이고 있어도,
     구체적 사실을 빠뜨리는 손실은 여전히 실재한다. 이건 '늘려 쓰라' 가 아니라
     '있는 걸 버리지 말라' 다 — 지어낼 필요가 없다. */
  '- 캡션에 있는 **구체적 사실은 하나도 버리지 않는다**: 브랜드·인물·제품명,',
  '  날짜·기간, 장소, 가격, 수치, 컬렉션명, 크레딧, 참여자 명단.',
  '  캡션의 영문 단락은 한국어 단락의 번역이다. 새 사실이 아니므로 중복해 쓰지 않는다.',
  '  다만 영문에만 있는 고유명사·수치가 있으면 그것은 가져온다.',
  /* 2026-08-26 GEO — 정의형 문장(definitional sentence)은 검색엔진·AI 답변이
     문장 단위로 그대로 발췌하는 최상위 인용 단위다. PAP 평서체("~다")와
     충돌하지 않고, 지어내기 금지 원칙 아래에서만 쓴다. */
  '- 트렌드·기법·스타일 용어가 처음 등장하면 그 자리에서 한 문장으로 정의한다:',
  '  "발레코어는 발레 연습복의 실루엣을 일상복으로 옮긴 스타일이다."',
  '  글당 최대 1~2회. 확실히 아는 일반 지식만 정의하고, 모르는 용어는 정의 없이 그대로 쓴다.',
  /* 2026-08-27 (도메니코 지시): "정량화할 수 있는 건 항상 숫자로 표시" —
     숫자가 든 문장은 AI 답변이 문장 단위로 그대로 인용하는 최상위 단위다. */
  '- 정량화할 수 있는 사실은 어림말 대신 숫자로 쓴다: "여러 룩" 대신 "룩 6개",',
  '  "오랜만의 컴백" 대신 "2년 만의 컴백", "많은 브랜드" 대신 "브랜드 12곳".',
  '  단, 재료(캡션·이미지)에 있는 숫자만 — 숫자를 지어내는 것은 최악의 위반이다.',
].join('\n');
...
const EN_BODY_ARTICLE = enBody('- 4 to 5 paragraphs, matching the Korean paragraph count exactly.');
...
/* 자가검증도 길이 규격을 따라간다 (2026-08-17).
   본문에는 "600~800자" 라고 해놓고 자가검증에 다른 숫자를 남기면
...
const SELF_CHECK_ARTICLE = selfCheck(
  '- 단락이 6개 이상이다 / 1,000자를 넘는다 / 600자에 한참 못 미치는데 더 쓸 사실이 있다');
...
/** 웹사이트 기사 생성기용 풀 스펙 — 본문 600~800자 (2026-08-22 현실화).
 *  08-17 에 800~1,200 으로 올렸다가, 한국어 재료가 평균 350자뿐이라는 실측으로
 *  되내렸다. 근거는 위 LENGTH_ARTICLE 주석.
...
function lintKoreanBody(text, opts) {
  ...
  if (structure) {
    /* 한도를 인자로 받는다 (2026-08-17).
       ...
       { maxParas: 5, maxLen: 1100 } 로 넘겨 새 규격을 쓴다.
       520 은 450 권장 상한에 15% 여유를 준 값이었다. 기사 상한이 2026-08-22 에
       1,400 → 1,000자로 내려갔으므로 같은 비율로 1,100 을 기사 한도로 둔다. */
