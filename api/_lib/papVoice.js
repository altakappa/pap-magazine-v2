...
const SOCIAL_VOICE = [
  ...
  '- 독자를 부르지 않는다. 호칭도 2인칭 대명사도 쓰지 않는다.',
  '  주어를 지우고 바로 말한다. 스레드는 원래 그렇게 쓴다.',
...
const X_VOICE = [
  ...
  '  ※ 하지 말라는 표현을 예시로 적지 않는다. 모델이 그 문구를 오히려 집는다.',
  // 2026-08-13 도메니코 지시 — 스레드와 같이 호칭을 없앤다.
  '- 독자를 부르지 않는다. 호칭도 2인칭 대명사도 쓰지 않는다.',
  '  주어를 지우고 바로 말한다. 부르는 순간 공지문이 된다.',
  '- 질문으로 끝내는 것은 기본값이 아니다. 정말 답이 궁금한 지점이 있을 때만 묻는다.',
  '  그런 지점이 없으면 여운 있는 한 줄이나 관찰로 닫는다.',
  '  물음표 없이 끝나는 글이 절반쯤 되는 것이 자연스럽다. 억지 질문은 대화를 만들지 못한다.',
 ...
 *
 * 2026-08-13 도메니코 지시 — 독자 호칭을 없앤다.
 * 그전(2026-08-03)에는 "패퍼들"로 불렀다. 그런데 실제 스레드에서 반응이
 * 좋은 글들은 **독자를 아예 부르지 않는다**. 주어를 지우고 바로 말한다.
 * 호칭이 붙는 순간 "구독자 여러분" 같은 방송 말투가 되고, 스레드에서
 * 그건 광고로 읽힌다.
 *
 * 그래서 두 갈래로 처리한다:
 *   ① 주어 자리(너는/너도/너가/너희는 …) → **지운다**. 호칭 없이 바로 말한다.
 *   ② 목적·소유 자리(너를/너의/너한테 …) → "다들"로 바꾼다.
 *      지워버리면 문장이 무너지는 자리라 대체어가 필요하다.
 *   ③ 옛 호칭(패퍼들·페퍼들)이 모델 출력에 남아 나오면 같은 규칙으로 정리한다.
 *      프롬프트에서 뺐어도 학습된 습관은 한동안 샌다.
 *
...
const READER_ADDRESS = [
  /* ① 주어 자리 — 지운다. 뒤따르는 공백까지 같이 먹어야 두 칸이 남지 않는다. */
  [/(^|[^가-힣])패퍼들은\s*/g, '$1'],
  [/(^|[^가-힣])페퍼들은\s*/g, '$1'],
  [/(^|[^가-힣])너희들은\s*/g, '$1'],
  [/(^|[^가-힣])너희는\s*/g, '$1'],
