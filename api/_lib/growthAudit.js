...
// 판정 규칙은 의존 없는 파일에 둔다 — 테스트가 supabase 클라이언트 생성 없이
// 규칙만 검증할 수 있어야 한다 (2026-07-30 CI: Node 20 에 전역 WebSocket 이 없어
// createClient() 가 import 시점에 죽었고, Node 22 인 로컬만 통과했다).
const { judgePace } = require('./pace');
...
async function runGrowthAudit() {
  ...
  // ── B. 발행 페이스 ────────────────────────────────────────────
  /* 2026-07-30 판정 수정 (도메니코 지적) — 공급이 없으면 미달이 아니다.
   *
   * 필름·에디토리얼은 서브미션(투고)이 들어와야 만드는 채널이다. 투고가 없는
   * 주에 0건인 것은 정상 운영이지, 고쳐야 할 고장이 아니다. 그런데 고정 주간
   * 할당량만 보고 판정하니 films_pace 가 매일 '긴급' 으로 떴다.
   * 매일 뜨는 긴급은 사람이 대시보드 전체를 안 믿게 만든다 — 참여지표 MIN_WOW
   * 때 배운 것과 같은 실수다(2026-07-25).
   *
   * 그래서 '만들 거리가 있었는데 못 냈는가' 를 본다:
   *   대기 소재 = 미발행 draft + 심사중(pending)·보완요청(revision) 서브미션
   *   대기 0 → ok (공급 없음). 대기 있음 → 기존 목표 기준으로 warn/fail.
   * 기사(articles)는 IG 자동수입이라 공급이 자동이므로 종전대로 목표만 본다. */
  const pace = (table, label, weeklyTarget, supplyFn) =>
    check(`${table}_pace`, label, async () => {
      ...
      // 공급 조회는 목표 미달일 때만 — 정상인 날에 쿼리 2개를 더 쏘지 않는다.
      const waiting = (supplyFn && last7 < weeklyTarget) ? await supplyFn() : null;
  ...
  sections.cadence = await Promise.all([
    ...
    pace('articles', '기사 주간 발행', 5), // IG 자동수입 — 공급이 자동이라 목표만 본다
    pace('films', '필름 주간 발행', 1, waitingSupply('films')),
  ]);
...
module.exports = { runGrowthAudit, judgePace };
