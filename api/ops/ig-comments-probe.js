...
module.exports = async function handler(req, res) {
  ...
  /* 살포 가산 — 보조 신호지 단독 근거가 아니다.
   *
   * 2026-08-19 두 번째 오탐: '@pap_magazine 🤍' 같은 멘션 댓글 4건이 정확히
   * 60점으로 스팸 판정을 받았다. 전부 살포 가산 +60 하나뿐이었고 구조 신호도
   * 키워드도 0이었다. 여러 팬이 우리를 태그하면 정규화 후 같은 문자열이 되니
   * 당연히 같은 지문이 된다. 그건 살포가 아니라 사랑이다.
   *
   * 그리고 이건 숨기면 가장 아까운 댓글이다 — 우리를 태그한 사람들이다.
   *
   * 그래서 규칙을 바꾼다: 스스로 아무 신호도 못 낸 댓글에는 살포 가산을
   * 주지 않는다. '남들도 똑같이 썼다'는 사실만으로는 스팸이 아니다.
   * 진짜 스패머는 구조 신호로 이미 175점 이상을 받으므로 영향이 없다. */
  for (const j of judged) {
    if (!j.지문) continue;                        // 묶을 근거가 없는 것은 가산하지 않는다
    if (j.점수 <= 0) continue;                    // 자기 신호가 0이면 살포 가산도 없다
