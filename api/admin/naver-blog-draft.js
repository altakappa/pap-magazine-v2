...
const FRAMEWORK_BLOCK = [
  ...
  '   • 이모지는 전체 최대 3개',
  '',
  /* 2026-08-27 (도메니코 제안) — 네이버 AI 브리핑 인용 최적화.
   * 근거: AI 브리핑 인용 272건 분석에서 블로그가 158건으로 최다 출처였고,
   * 인용되는 글의 공통 구조가 확인됨 — 답 먼저(도입 200자)·질문형 소제목·
   * 구간 독립성·FAQ 구조(인용률 3.2배)·출처 명시. 구조는 통일하되 문장 표현은
   * 글마다 달라야 한다(유사문서 판정 방지). */
  '3-1) AI 인용 최적화 (네이버 AI 브리핑·검색 AI가 발췌하는 구조):',
  '   • 도입부 훅 3문장 안에 이 글의 **핵심 답 한 문장**을 넣는다. 용어·트렌드가',
  '     주제면 정의형("X는 ~이다")으로. AI는 첫 200자에서 답을 찾는다.',
  '   • h3 소제목 중 2개 이상은 **질문형**("~인 이유는?", "~은 무엇이 다를까?")으로 쓰고,',
  '     그 구간의 **첫 1~2문장이 곧 답**이 되게 한다 (답 먼저, 설명은 뒤에).',
  '   • 각 h3 구간은 따로 떼어 읽어도 완결되게 — AI는 글 전체가 아니라 구간 단위로 발췌한다.',
  '   • 체크리스트 바로 위에 <h3>자주 묻는 질문</h3> 아래 Q&A 2문항: 질문은 사람들이',
  '     실제 검색창에 칠 말로, 답은 2~3문장(핵심 답을 첫 문장에). 본문 내용의 재정리만 —',
  '     본문에 없는 새 주장 금지.',
  '   • 전시명·날짜·장소·작가명 같은 구체 사실은 그대로 명시한다 (출처가 있는 글이 인용된다).',
  '   • 정량화할 수 있는 사실은 어림말 대신 **숫자로** 쓴다: "여러 작품" 대신 "작품 14점",',
  '     "오랜 기간" 대신 "6년간". 숫자가 든 문장이 AI 발췌 1순위다. 단 원문에 있는 숫자만 —',
  '     숫자를 지어내는 것은 절대 금지.',
  '   • 이 구조(답 먼저·질문형 소제목·Q&A)는 매 글 유지하되, **문장 표현·어휘는 글마다',
  '     다르게** — 표현까지 획일화되면 유사문서로 묶여 노출이 죽는다.',
  '',
