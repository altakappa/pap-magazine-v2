  ...
  try {
    ...
    const tpl = templates.submissionReviewComplete(
      ...
      { approvalDay: opts.approvalDay || '', approvalMonth: opts.approvalMonth || '',
        feeCents: _feeCents,
        isPremium: hasActivePlan(profile, 'premium'),   // 2026-09-13 비프리미엄에게만 업셀 블록
  ...
  if (req.method === 'GET') {
    try {
      ...
      try {
         ...
         * 판정 자체는 canView 하나를 그대로 쓴다(등급표를 두 벌로 만들지
         * 않는다). 열람 권한이 있으면 전체, 없으면 앞 2장. */
        const shaped = edAccess.shapeGallery(data, tier, { freeIds });
